...
export class BattleEngineServer {
  ...
  executeAction(action: BattleAction): {
    ...
    if (action.type === 'attack' && target) {
      ...
      console.log(
        `[BattleEngineServer] Base damage: ${baseDamage} (attack: ${attackPower}, defense: ${defensePower})`
      );
  ...
  private nextTurn() {
    this.turnIndex++;
    console.log(
      `[BattleEngineServer] nextTurn() - turnIndex now: ${this.turnIndex}, turnOrder length: ${this.turnOrder.length}`
    );
