...
export class PvPBattleRoom extends Room<BattleRoomState> {
  ...
  onCreate(options: any) {
    ...
    this.onMessage('accept-settings', (client, settings) =>
      this.handleAcceptSettings(client, settings)
    );
    this.onMessage('propose-settings', (client, settings) =>
      this.handleProposeSettings(client, settings)
    );
  ...
  onJoin(client: Client, options: any) {
    ...
    player.address = isTestWallet
      ? `${options.address}_${client.sessionId.slice(0, 6)}`
      : options.address || '0x0000';
    ...
    if (this.state.players.size === 2) {
      ...
      const settingsMatch =
        prefs[0].teamSize === prefs[1].teamSize && prefs[0].speed === prefs[1].speed;
      ...
      if (settingsMatch) {
        ...
        players.forEach(([id, player]) => {
          ...
          const otherPrefs = Array.from(this.playerPreferences.entries()).find(
            ([otherId]) => otherId !== id
          )?.[1]!;

          this.clients
            .find((c) => c.sessionId === id)
            ?.send('settings-mismatch', {
  ...
  handleReady(client: Client) {
    ...
    player.ready = true;
    console.log(
      `[PvP] Player ${client.sessionId} is ready. Players: ${this.state.players.size}, All ready: ${Array.from(this.state.players.values()).every((p) => p.ready)}`
    );
  ...
  handleAction(client: Client, action: any) {
    ...
    if (this.state.currentPhase && this.state.currentPhase !== 'attack-timing') {
      console.log(
        `[PvP Server] Action received but not in attack-timing phase: ${this.state.currentPhase}`
      );
      return;
    }
  ...
  private nextTurn(): void {
    ...
    const currentUnit = this.battleEngine.getCurrentUnit();
    console.log(
      `[PvP] nextTurn() - current unit: ${currentUnit?.id}, owner: ${currentUnit?.ownerId}`
    );
    if (!currentUnit) {
    ...
    // Notify players
    console.log(
      `[PvP] Broadcasting turn-start for unit ${currentUnit.id}, owner ${currentUnit.ownerId}, phase: selecting-action`
    );
  ...
  private handlePhaseTimeout() {
    ...
    switch (this.state.currentPhase) {
      ...
      case 'selecting-target':
        ...
        const enemies = Array.from(this.state.units).filter(
          (u) => u.ownerId !== currentUnit.ownerId && u.isAlive
        );
