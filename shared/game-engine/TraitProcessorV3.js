...
exports.TraitProcessorV3 = void 0;
const elements_json_1 = __importDefault(require("../data/elements.json"));
const ability_combinations_json_1 = __importDefault(require("../data/ability-combinations.json"));
const abilities_v2_json_1 = __importDefault(require("../data/abilities-v2.json"));
const trait_element_mapping_json_1 = __importDefault(require("../data/trait-element-mapping.json"));
const real_ability_combinations_json_1 = __importDefault(require("../data/real-ability-combinations.json"));
const BASE_STATS = {
    hp: 100,
    attack: 50,
    defense: 50,
...
class TraitProcessorV3 {
    static processRobotoTraits(metadata) {
        ...
        return {
            ...
            traits,
            elementModifiers: {
                strongAgainst: elements_json_1.default.elements[element]?.strongAgainst || [],
                weakAgainst: elements_json_1.default.elements[element]?.weakAgainst || [],
            },
        };
    }
    static processRobopetTraits(metadata) {
        const traits = this.extractTraits(metadata.attributes || []);
        ...
        const petAbilities = ability_combinations_json_1.default.combinations.robopet_abilities[petType] ||
            ability_combinations_json_1.default.combinations.robopet_abilities.default;
    ...
    static determineElement(traits) {
        const elementScores = {
            SURGE: 0,
            CODE: 0,
            METAL: 0,
        ...
        const mappings = trait_element_mapping_json_1.default.trait_mappings;
        ...
        Object.entries(traits).forEach(([traitType, traitValue]) => {
            ...
            const realMapping = mappings[traitType]?.[traitValue];
            if (realMapping) {
                ...
                Object.entries(realMapping).forEach(([element, weight]) => {
                    if (element in elementScores) {
                        elementScores[element] += weight * multiplier;
                    }
                });
            }
            else if (veryImportantTraitTypes.includes(traitType) ||
                importantTraitTypes.includes(traitType)) {
                ...
                else if (value.includes('digital') ||
                    value.includes('data') ||
                ...
                else if (value.includes('glitch') ||
                    value.includes('error') ||
                    value.includes('corrupt') ||
                    value.includes('chaos') ||
                    value.includes('wild') ||
                    value.includes('crazy')) {
    ...
    static calculateStats(traits, element, baseStats) {
        const stats = { ...baseStats };
        // Apply element modifiers
        const elementData = elements_json_1.default.elements[element];
    ...
    static findUnlockedAbilities(traits, unitType) {
        const abilities = [];
        if (unitType === 'roboto') {
            // Always add basic ability based on element
            const element = this.determineElement(traits);
            const basicAbility = ability_combinations_json_1.default.combinations.basic_abilities[element];
            if (basicAbility) {
                abilities.push(basicAbility.id);
            }
            // Check real NFT trait combinations first
            real_ability_combinations_json_1.default.combinations.forEach((combo) => {
                const hasAllRequiredTraits = Object.entries(combo.requirements).every(([traitType, requiredValue]) => traits[traitType] === requiredValue);
            ...
            ability_combinations_json_1.default.combinations.robotos.forEach((combo) => {
                const hasAllRequiredTraits = Object.entries(combo.requirements).every(([traitType, requiredValue]) => traits[traitType] === requiredValue);
                if (hasAllRequiredTraits && !abilities.includes(combo.id)) {
                    abilities.push(combo.id);
                }
            ...
            if (abilities.length > 5) {
                ...
                const prioritized = abilities.sort((a, b) => {
                    const comboA = ability_combinations_json_1.default.combinations.robotos.find((c) => c.id === a);
                    const comboB = ability_combinations_json_1.default.combinations.robotos.find((c) => c.id === b);
                    const rarityOrder = { common: 0, rare: 1, epic: 2, legendary: 3 };
                    const rarityA = comboA ? rarityOrder[comboA.rarity] || 0 : 0;
                    const rarityB = comboB ? rarityOrder[comboB.rarity] || 0 : 0;
    ...
    static getAbilityData(abilityId) {
        // Search through all ability categories
        const allAbilities = {
            ...abilities_v2_json_1.default.basic_abilities,
            ...abilities_v2_json_1.default.real_trait_abilities,
            ...abilities_v2_json_1.default.surge_abilities,
            ...abilities_v2_json_1.default.code_abilities,
            ...abilities_v2_json_1.default.metal_abilities,
            ...abilities_v2_json_1.default.glitch_abilities,
            ...abilities_v2_json_1.default.legendary_abilities,
            ...abilities_v2_json_1.default.robopet_abilities,
        };
        return allAbilities[abilityId] || null;
    }
    static calculateDamage(attacker, defender, ability) {
        ...
        const elementMultiplier = this.getElementMultiplier(ability.element || attacker.element, defender.element);
    ...
    static getElementMultiplier(attackElement, defenseElement) {
        ...
        if (!typeChart)
            return 1.0;
        return typeChart[defenseElement.toLowerCase()] || 1.0;
    }
    static getElementColor(element) {
        return elements_json_1.default.elements[element]?.color || '#FFFFFF';
    }
    static getElementSymbol(element) {
        return elements_json_1.default.elements[element]?.symbol || '?';
    }
    static getElementAttackNames(element) {
        const attackNames = {
        ...
        return attackNames[element.toUpperCase()] || attackNames['NEUTRAL'];
    }
}
exports.TraitProcessorV3 = TraitProcessorV3;
