...
export default function RobotoUnit({
  ...
  const elementColor =
    ELEMENT_COLORS[unit.element as keyof typeof ELEMENT_COLORS] ||
  ...
  return (
    ...
    >
        ...
        <div className="absolute bottom-0 left-0 right-0 h-2 bg-black/80 rounded-b-xl">
          <motion.div
            ...
            className={cn(
              ...
              currentHp && maxHp && currentHp > maxHp * 0.5
                ? 'bg-green-400'
                : currentHp && maxHp && currentHp > maxHp * 0.25
                  ? 'bg-yellow-400'
                  : 'bg-red-400'
            )}
          />
      ...
      <div className="absolute -bottom-12 left-0 right-0 text-center space-y-1 pointer-events-none">
        ...
        <p className={cn('text-xs font-bold', elementColor)}>{unit.element || 'NEUTRAL'}</p>
      </div>
