...
export default function TimingMeterSpinner({
  ...
  const calculateScore = useCallback((angle: number) => {
    ...
    if ((normalizedAngle >= 350 && normalizedAngle <= 360) || (normalizedAngle >= 0 && normalizedAngle <= 10)) {
      return 1.5 // PERFECT
    }
    // Good zones: 330-350 and 10-30 (40 degree range total)
    else if ((normalizedAngle >= 330 && normalizedAngle < 350) || (normalizedAngle > 10 && normalizedAngle <= 30)) {
      return 1.25 // GOOD
    }
    // OK zones: 300-330 and 30-60 (60 degree range total)
    else if ((normalizedAngle >= 300 && normalizedAngle < 330) || (normalizedAngle > 30 && normalizedAngle <= 60)) {
      return 1.0 // OK
    }
        ...
        <div className="relative w-48 h-48">
          ...
          <svg className="absolute inset-0 w-full h-full" viewBox="0 0 200 200">
            <defs>
              ...
              <linearGradient id="ringGradient" gradientUnits="userSpaceOnUse" x1="100" y1="0" x2="100" y2="200">
                <stop offset="0%" stopColor="rgb(34 197 94)" stopOpacity="0.5" />
                <stop offset="5%" stopColor="rgb(34 197 94)" stopOpacity="0.5" />
                <stop offset="10%" stopColor="rgb(250 204 21)" stopOpacity="0.4" />
                <stop offset="15%" stopColor="rgb(250 204 21)" stopOpacity="0.4" />
                <stop offset="20%" stopColor="rgb(251 146 60)" stopOpacity="0.3" />
                <stop offset="30%" stopColor="rgb(251 146 60)" stopOpacity="0.3" />
                <stop offset="40%" stopColor="rgb(239 68 68)" stopOpacity="0.2" />
                <stop offset="70%" stopColor="rgb(239 68 68)" stopOpacity="0.2" />
                <stop offset="80%" stopColor="rgb(251 146 60)" stopOpacity="0.3" />
                <stop offset="85%" stopColor="rgb(250 204 21)" stopOpacity="0.4" />
                <stop offset="90%" stopColor="rgb(250 204 21)" stopOpacity="0.4" />
            ...
            <g stroke="rgb(0 0 0 / 0.5)" strokeWidth="1">
              ...
              <line x1="100" y1="20" x2="100" y2="50" transform="rotate(10 100 100)" />
              <line x1="100" y1="20" x2="100" y2="50" transform="rotate(-10 100 100)" />
              {/* Good zone boundaries */}
              <line x1="100" y1="20" x2="100" y2="50" transform="rotate(30 100 100)" />
              <line x1="100" y1="20" x2="100" y2="50" transform="rotate(-30 100 100)" />
              {/* OK zone boundaries */}
              <line x1="100" y1="20" x2="100" y2="50" transform="rotate(60 100 100)" />
              <line x1="100" y1="20" x2="100" y2="50" transform="rotate(-60 100 100)" />
            </g>
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="text-[10px] font-bold">
              <div className="absolute top-2 left-1/2 -translate-x-1/2 text-green-400">PERFECT</div>
              ...
              <div className="absolute bottom-12 left-1/2 -translate-x-1/2 text-red-400">WEAK</div>
            </div>
          </div>
          ...
          >
            <div className="absolute w-1 h-24 -top-2">
              <div className={cn(
                ...
                hasInput ? (
                  score >= 1.5 ? 'bg-green-400' :
                  score >= 1.25 ? 'bg-yellow-400' :
                  score >= 1.0 ? 'bg-orange-400' :
                  'bg-red-400'
                ) : 'bg-white'
              )} />
              {/* Round end cap */}
      ...
      <div className="text-center">
        {hasInput ? (
          ...
          <div className="text-sm text-gray-400">
            Press <span className="text-white font-bold">SPACE</span> or <span className="text-white font-bold">ENTER</span> to stop!
          </div>
        )}
      </div>
    </motion.div>
