...
export type GameSoundType = 
  // UI Sounds
  | 'hover' | 'click' | 'select' | 'deselect' | 'cancel' | 'confirm'
  | 'error' | 'success' | 'warning' | 'notification'
  // Team Building
  | 'addUnit' | 'removeUnit' | 'teamComplete' | 'swapUnit'
  // Battle Sounds
  | 'attack' | 'defend' | 'ability' | 'critical' | 'miss' | 'damage'
  | 'victory' | 'defeat' | 'roundStart' | 'turnStart' | 'turnEnd'
  | 'heal' | 'buff' | 'debuff' | 'statusEffect'
  // Timing System
  | 'timingStart' | 'timingPerfect' | 'timingGood' | 'timingMiss'
  | 'timingLocked' | 'timingTick' | 'timingCountdown' | 'timingExpired'
  // Timing Meters
  | 'chargeTap' | 'chargeBuilding' | 'chargeComplete'
  | 'spinnerActive' | 'spinnerStop'
  // Menu
  | 'menuNavigate' | 'menuOpen' | 'menuClose' | 'tabSwitch'
  // Elements
  | 'surge' | 'code' | 'metal' | 'glitch'
  // Combat Feedback
  | 'lowHealth' | 'unitDefeat' | 'unitSwitch' | 'charge'
  | 'targetSelect' | 'targetConfirm' | 'energyRestore'
  | 'explosion'
  // Attack Variations
  | 'attackWeak' | 'attackNormal' | 'attackStrong' | 'attackDevastating'
  | 'criticalSuper' | 'criticalCombo'
  // Abilities
  | 'abilityReady' | 'abilityCooldown' | 'abilitySelect'
  | 'bladeSlash' | 'electricBurst' | 'airBurst' | 'crushImpact'
  | 'spinningSaw' | 'aerialStrike'
  // Turn Sounds
  | 'playerTurn' | 'enemyTurn' | 'turnCountdown'
  // Element Combos
  | 'elementCombo' | 'trinityBonus' | 'elementAdvantage';
...
class GameSoundPlayer {
  ...
  public playAttackSound(damage: number, isCritical: boolean = false, isPerfectTiming: boolean = false) {
    if (damage === 0) {
      this.play('miss');
    } else if (isCritical) {
      if (damage >= 100) {
