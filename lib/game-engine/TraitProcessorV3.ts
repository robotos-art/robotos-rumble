...
export class TraitProcessorV3 {
  ...
  static processRobopetTraits(metadata: any): BattleUnitV3 {
    ...
    const petAbilities =
      (abilityCombinations.combinations.robopet_abilities as any)[petType] ||
  ...
  private static determineElement(
    traits: Record<string, string>,
  ): "SURGE" | "CODE" | "METAL" | "GLITCH" | "NEUTRAL" {
    ...
    Object.entries(traits).forEach(([traitType, traitValue]) => {
      ...
      if (realMapping) {
        ...
        const multiplier = veryImportantTraitTypes.includes(traitType)
          ? 1.5
          : 1;
        ...
        veryImportantTraitTypes.includes(traitType) ||
        ...
        if (
          ...
          elementScores.SURGE += veryImportantTraitTypes.includes(traitType)
            ? 3
            : 2;
        } else if (
          ...
          elementScores.CODE += veryImportantTraitTypes.includes(traitType)
            ? 3
            : 2;
        } else if (
          ...
          elementScores.METAL += veryImportantTraitTypes.includes(traitType)
            ? 3
            : 2;
        } else if (
          value.includes("glitch") ||
          ...
          elementScores.GLITCH += veryImportantTraitTypes.includes(traitType)
            ? 3
            : 2;
        }
  ...
  private static calculateStats(
    ...
    if (traits["Backpack"] && backpackMods[traits["Backpack"]]) {
      Object.entries(backpackMods[traits["Backpack"]]).forEach(
        ([stat, value]) => {
          (stats as any)[stat] += value;
        },
      );
    ...
    if (traits["Cyborgo Face"] && cyborgoFaceMods[traits["Cyborgo Face"]]) {
      Object.entries(cyborgoFaceMods[traits["Cyborgo Face"]]).forEach(
        ([stat, value]) => {
          (stats as any)[stat] += value;
        },
      );
    ...
    if (traits["Computo Head"]) {
      const mod =
        computoHeadMods[traits["Computo Head"]] || computoHeadMods["Default"];
    ...
    if (traits["Robot Type"] && typeMods[traits["Robot Type"]]) {
      Object.entries(typeMods[traits["Robot Type"]]).forEach(
        ([stat, value]) => {
          (stats as any)[stat] += value;
        },
      );
    ...
    if (traits["Special"]) {
      ...
      const specials = Array.isArray(traits["Special"])
        ? traits["Special"]
        : [traits["Special"]];
  ...
  private static findUnlockedAbilities(
    ...
    if (unitType === "roboto") {
      ...
      const element = this.determineElement(traits);
      const basicAbility = (
        abilityCombinations.combinations.basic_abilities as any
      )[element];
      if (basicAbility) {
      ...
      if (abilities.length > 5) {
        ...
        const prioritized = abilities.sort((a, b) => {
          const comboA = abilityCombinations.combinations.robotos.find(
            (c) => c.id === a,
          );
          const comboB = abilityCombinations.combinations.robotos.find(
            (c) => c.id === b,
          );

          const rarityOrder = { common: 0, rare: 1, epic: 2, legendary: 3 };
          const rarityA = comboA
            ? rarityOrder[comboA.rarity as keyof typeof rarityOrder] || 0
            : 0;
          const rarityB = comboB
            ? rarityOrder[comboB.rarity as keyof typeof rarityOrder] || 0
            : 0;
  ...
  static calculateDamage(
    ...
    const defenseReduction =
      defender.stats.defense / (defender.stats.defense + 100);
