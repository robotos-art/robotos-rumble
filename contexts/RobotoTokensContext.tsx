...
function getCachedTokens(
  address: string
): { robotos: TokenWithMetadata[]; robopets: TokenWithMetadata[]; timestamp: number } | null {
  try {
...
export function RobotoTokensProvider({ children }: { children: React.ReactNode }) {
    ...
    try {
      ...
      const fetchMetadataBatch = async (tokenIds: string[], type: 'roboto' | 'robopet') => {
        ...
        const promises = tokenIds.map(async (tokenId) => {
          try {
            ...
            const response = await fetch(`https://ipfs.io/ipfs/${ipfsHash}/${tokenId}`, {
      ...
      for (let i = 0; i < robotoIds.length; i += BATCH_SIZE) {
        ...
        setRobotos((prev) => {
          ...
          const uniqueTokens = newTokens.filter(
            (token, index, self) => index === self.findIndex((t) => t.tokenId === token.tokenId)
          );
          return uniqueTokens;
        });
      }
  ...
  return <RobotoTokensContext.Provider value={value}>{children}</RobotoTokensContext.Provider>;
}
