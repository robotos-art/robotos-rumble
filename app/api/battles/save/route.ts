...
export async function POST(request: NextRequest) {
  try {
    ...
    const totalDuration =
      profile.stats.averageBattleDuration * oldStats.totalBattles + battleData.duration;
    ...
    const mostUsedElement = Object.entries(elementCounts).sort(([, a], [, b]) => b - a)[0]?.[0];
    if (mostUsedElement) {
    ...
    const newAchievements = checkAchievements(battleData, profile.stats, profile.achievements);
