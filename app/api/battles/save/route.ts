...
export async function POST(request: NextRequest) {
  try {
    ...
    const totalDuration =
      profile.stats.averageBattleDuration * oldStats.totalBattles +
      battleData.duration;
    profile.stats.averageBattleDuration = Math.round(
      totalDuration / profile.stats.totalBattles,
    );
    ...
    const mostUsedElement = Object.entries(elementCounts).sort(
      ([, a], [, b]) => b - a,
    )[0]?.[0];
    if (mostUsedElement) {
    ...
    if (battleData.result === "victory") {
      ...
      if (hasPairedTeam) {
        profile.stats.pairedVictories =
          (profile.stats.pairedVictories || 0) + 1;
      }
    }
    ...
    // Battle count badges
    if (
      profile.stats.totalBattles >= 10 &&
      !profile.badges.some((b) => b.id === "recruit")
    ) {
      profile.badges.push({
        id: "recruit",
        tier: "bronze",
    ...
    if (
      profile.stats.totalBattles >= 50 &&
      !profile.badges.some((b) => b.id === "soldier")
    ) {
      profile.badges.push({
        id: "soldier",
        tier: "silver",
    ...
    if (
      profile.stats.totalBattles >= 100 &&
      !profile.badges.some((b) => b.id === "veteran")
    ) {
      profile.badges.push({
        id: "veteran",
        tier: "gold",
